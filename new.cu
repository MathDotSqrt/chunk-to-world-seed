...
__device__ void add_world_seed(uint64_t firstAddend, int32_t multTrailingZeroes, uint64_t firstMultInv, uint64_t c,
  ...
  if (multTrailingZeroes != 0) {
    ...
    uint64_t target = (((b ^ (bottom32BitsChunkseed >> 16)) & smallMask) -
                       (get_partial_addend((b << 16) + c, 32 - multTrailingZeroes) >> 16)) &
                      smallMask;
...
__global__ void crack(uint64_t seedInputCount, uint64_t *seedInputArray, uint64_t *seedOutputCounter,
  ...
  if (CHUNK_X == 0 && CHUNK_Z == 0) {
    ...
    for (; c < (1ULL << 16); c += (1ULL << (totalCount + 1))) {
      ...
      add_world_seed(target - (magic & MASK16), multTrailingZeroes, firstMultInv, c, chunkSeed, seedOutputArray,
                     seedOutputCounter);
      if (CHUNK_X != 0) {
        add_world_seed(target - ((magic + x) & MASK16), multTrailingZeroes, firstMultInv, c, chunkSeed, seedOutputArray,
                       seedOutputCounter);
      }
      if (CHUNK_Z != 0 && CHUNK_X != CHUNK_Z) {
        add_world_seed(target - ((magic + z) & MASK16), multTrailingZeroes, firstMultInv, c, chunkSeed, seedOutputArray,
                       seedOutputCounter);
      }
      if (CHUNK_X != 0 && CHUNK_Z != 0 && CHUNK_X + CHUNK_Z != 0) {
        add_world_seed(target - ((magic + x + z) & MASK16), multTrailingZeroes, firstMultInv, c, chunkSeed,
                       seedOutputArray, seedOutputCounter);
      }
      if (CHUNK_X != 0 && CHUNK_X != CHUNK_Z) {
        add_world_seed(target - ((magic + 2 * x) & MASK16), multTrailingZeroes, firstMultInv, c, chunkSeed,
                       seedOutputArray, seedOutputCounter);
      }
      if (CHUNK_Z != 0 && CHUNK_X != CHUNK_Z) {
        add_world_seed(target - ((magic + 2 * z) & MASK16), multTrailingZeroes, firstMultInv, c, chunkSeed,
                       seedOutputArray, seedOutputCounter);
      }
      if (CHUNK_X != 0 && CHUNK_Z != 0 && CHUNK_X + CHUNK_Z != 0 && CHUNK_X * 2 + CHUNK_Z != 0) {
        add_world_seed(target - ((magic + 2 * x + z) & MASK16), multTrailingZeroes, firstMultInv, c, chunkSeed,
                       seedOutputArray, seedOutputCounter);
      }
      if (CHUNK_X != 0 && CHUNK_Z != 0 && CHUNK_X != CHUNK_Z && CHUNK_X + CHUNK_Z != 0 && CHUNK_X + CHUNK_Z * 2 != 0) {
        // is the x supposed to be multiplied
        add_world_seed(target - ((magic + x + 2 * z) & MASK16), multTrailingZeroes, firstMultInv, c, chunkSeed,
                       seedOutputArray, seedOutputCounter);
      }
      if (CHUNK_X != 0 && CHUNK_Z != 0 && CHUNK_X + CHUNK_Z != 0) {
        add_world_seed(target - ((magic + 2 * x + 2 * z) & MASK16), multTrailingZeroes, firstMultInv, c, chunkSeed,
                       seedOutputArray, seedOutputCounter);
      }
    }
  }
